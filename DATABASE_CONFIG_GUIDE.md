...
### 39. knowledge_articles
**用途**：知识库文章管理

#### 📝 创建集合
- **集合名称**：`knowledge_articles`
- **权限类型**：选择"所有用户可读、创建者可读写"
  - 知识库文章供所有用户查看学习，但只有管理员可以创建和编辑

#### 📊 添加索引

**索引1：分类和时间索引**
- 索引名称：`category_1_createTime_-1`
- 索引属性：非唯一
- 索引字段：
  - 字段：`category`，排序：升序
  - 字段：`createTime`，排序：降序
...
- `description`：文章描述
- `content`：文章内容
- `category`：分类（all/breeding/disease/breed/market）
- `categoryName`：分类名称（全部/饲养技术/疾病防治/品种介绍/市场分析）
- `categoryTheme`：分类主题色（default/primary/success/warning/danger）
- `views`：阅读量（字符串类型，默认'0'）
- `readTime`：预计阅读时间（分钟，字符串类型，根据内容自动计算）
- `date`：发布日期（格式：MM-DD）
- `operator`：操作员（创建/更新者）
...
### ✅ 权限配置检查
- [ ] 仅创建者可读写：4个集合
- [ ] 所有用户可读、创建者可读写：22个集合（包含knowledge_articles）
- [ ] 所有用户可读：10个集合
- [ ] 所有用户不可读写：6个集合
...
**更新时间**：2025年11月7日  
**适用项目**：鹅数通智慧养鹅小程序  
**最新更新**：新增知识库管理模块（knowledge_articles集合）
