...
Page<PageData, any>({
  ...
  async loadTodayTasks() {
    ...
    try {
      ...
      // 并行加载所有批次的任务
      const batchTasksPromises = batches.map(async (batch: unknown) => {
        try {
          ...
          if (response.success && response.data && response.data.length > 0) {
            ...
            const normalizedTasks = tasks.map((task: unknown) =>
      ...
      // 收集所有任务
      let allTasks: Batch[] = []
      validBatchTasks.forEach((batchData: unknown) => {
   ...
   * 诊断记录点击事件 - 使用公共工具函数处理
   */
  async onDiagnosisRecordTap(e: CustomEvent) {
   ...
   * 预览图片
   */
  onPreviewDiagnosisImage(e: CustomEvent) {
   ...
   * 点击治疗记录，跳转到详情页
   */
  onTreatmentRecordTap(e: CustomEvent) {
    if (this.checkDoubleClick && this.checkDoubleClick()) return
   ...
   * 查看预防记录详情
   */
  viewPreventionRecord(e: CustomEvent) {
   ...
   */
  async onPreventionSubTabChange(e: CustomEvent) {
    const { value } = e.detail
  ...
  async loadUpcomingTasks() {
    ...
    try {
      ...
      // 并行加载所有批次的即将到来任务
      const promises = batches.map(async (batch: unknown) => {
        try {
  ...
  async loadHistoryTasks() {
    try {
      ...
      if (result.data && result.data.length > 0) {
        ...
        const completedTasks: CompletedTaskItem[] = result.data.map((task: unknown): CompletedTaskItem => {
...
${record.taskId ? '\n来源：待办任务' : ''}
   ...
   * 预防管理操作事件
   */
  onPreventionAction(e: CustomEvent) {
   ...
   * 监控操作事件
   */
  onMonitoringAction(e: CustomEvent) {
    const { action } = e.currentTarget.dataset
    // 已移除调试日志
    switch (action) {
      case 'batch_check':
   ...
   * 治疗操作事件
   */
  onTreatmentAction(e: CustomEvent) {
    const { action } = e.currentTarget.dataset
    // 已移除调试日志
    switch (action) {
   ...
   */
  onHealthDetailPopupChange(e: CustomEvent) {
   ...
   */
  async viewTaskDetail(e: CustomEvent) {
    if (this.checkDoubleClick && this.checkDoubleClick()) return
   ...
   */
  onTaskDetailPopupChange(e: CustomEvent) {
    const { visible } = e.detail
    if (!visible) {
      this.setData({
        showTaskDetailPopup: false
   ...
   * 用药鹅只数量输入处理（适配组件事件）
   */
  onMedicationAnimalCountInput(e: CustomEvent) {
    const { value } = e.detail || {}
   ...
   * 症状等级选择处理（适配组件事件）
   */
  onSeverityChange(e: CustomEvent) {
    const index = e.detail?.index ?? e.detail?.value ?? 0
    this.setData({
