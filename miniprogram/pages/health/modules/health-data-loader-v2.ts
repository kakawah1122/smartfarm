...
export class HealthDataLoader {
   ...
   */
  async loadHealthOverview(options: DataLoaderOptions = {}): Promise<any> {
    ...
    if (!forceRefresh && useCache) {
      if (CacheManager.isCacheValid(cacheKey)) {
        return CacheManager.getCachedData(cacheKey) as any
      }
    }
   ...
   * 实际获取健康概览数据
   */
  private async fetchHealthOverview(batchId: string): Promise<any> {
    try {
   ...
   * 加载预防管理数据
   */
  async loadPreventionData(options: DataLoaderOptions = {}): Promise<PreventionData> {
    const { batchId = 'all' } = options
    const MAX_RETRIES = 2
    
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        const result = await HealthCloud.prevention.getDashboard({ batchId,
        ...
        return this.normalizePreventionData(result.data) as PreventionData
      } catch (error) {
    ...
    // 返回默认数据
    return this.getDefaultPreventionData() as PreventionData
  }
  
  /**
   * 标准化预防数据
   */
  private normalizePreventionData(data: any): PreventionData {
    return {
   ...
   * 获取默认预防数据
   */
  private getDefaultPreventionData(): PreventionData {
    return {
      todayTasks: [],
      upcomingTasks: [],
      stats: {
   ...
   */
  async loadTreatmentData(options: DataLoaderOptions = {}): Promise<TreatmentData> {
    
    try {
      // 获取健康概览数据，其中包含治疗相关数据
      const healthData = await this.loadHealthOverview(options) as any
      ...
      return this.getDefaultTreatmentData() as TreatmentData
    }
  }
   ...
   * 获取默认治疗数据
   */
  private getDefaultTreatmentData(): TreatmentData {
    return {
      totalCost: 0,
      totalTreatments: 0,
      recoveredCount: 0,
   ...
   */
  async loadAnalysisData(options: DataLoaderOptions = {}): Promise<AnalysisData> {
    try {
      // 获取健康概览数据
      const healthData = await this.loadHealthOverview(options) as any
      ...
      return this.getDefaultAnalysisData() as AnalysisData
    }
  }
   ...
   * 获取默认分析数据
   */
  private getDefaultAnalysisData(): AnalysisData {
    return {
      survivalRate: { rate: '-' },
      healthTrends: [],
      costAnalysis: {
   ...
   * 加载监测数据（从健康概览数据中提取）
   */
  async loadMonitoringData(options: DataLoaderOptions = {}): Promise<MonitoringData> {
    try {
      // 获取健康概览数据
      const healthData = await this.loadHealthOverview(options) as any
      ...
      return this.getDefaultMonitoringData() as MonitoringData
    }
  }
   ...
   * 获取默认监测数据
   */
  private getDefaultMonitoringData(): MonitoringData {
    return {
      realTimeStatus: {
        totalAnimals: 0,
        healthyCount: 0,
