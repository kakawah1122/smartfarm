...
Page({
    ...
    // 调用完整天气API
    callCompleteWeatherAPI(lat, lon) {
    ...
    // 更新完整天气数据
    updateCompleteWeatherData(weatherData) {
    ...
    processHourlyForecast(hourlyData) {
        ...
        const hourlyForecast = hourlyData.slice(0, 24).map((item, index) => {
    ...
    processDailyForecast(dailyData) {
        ...
        const dailyForecast = dailyData.slice(0, 7).map((item, index) => {
    ...
    // 处理天气预警数据
    processWeatherWarning(warningData) {
