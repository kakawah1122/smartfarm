...
| 索引名称 | 字段 | 排序 | 唯一 | 说明 |
|----------|------|------|------|------|
| `batchNumber_1` | batchNumber | 升序 | 是 | 批次号唯一 |
| `status_1_createTime_-1` | status, createTime | 升序, 降序 | 否 | 状态筛选 |
| `isActive_1` | isActive | 升序 | 否 | 活跃批次筛选 |
...
| 索引名称 | 字段 | 排序 | 唯一 | 说明 |
|----------|------|------|------|------|
| `treatmentRecordId_1` | treatmentRecordId | 升序 | 否 | 关联治疗记录 |
| `cureDate_-1` | cureDate | 降序 | 否 | 治愈日期排序 |
...
| 索引名称 | 字段 | 排序 | 唯一 | 说明 |
|----------|------|------|------|------|
| `treatmentRecordId_1_followupDate_-1` | treatmentRecordId, followupDate | 升序, 降序 | 否 | 治疗随访记录 |
...
| 索引名称 | 字段 | 排序 | 唯一 | 说明 |
|----------|------|------|------|------|
| `alertType_1_status_1_createTime_-1` | alertType, status, createTime | 升序, 升序, 降序 | 否 | 预警列表筛选 |
| `batchId_1` | batchId | 升序 | 否 | 批次预警查询 |

---

#### `health_vaccine_plans` - 疫苗计划表
...
| 索引名称 | 字段 | 排序 | 唯一 | 说明 |
|----------|------|------|------|------|
| `batchId_1_plannedDate_1` | batchId, plannedDate | 升序, 升序 | 否 | 批次疫苗计划 |
| `status_1_plannedDate_1` | status, plannedDate | 升序, 升序 | 否 | 待执行计划 |
...
| 常量名 | 实际集合名 | 问题 | 建议 |
|--------|------------|------|------|
| `PRODUCTION_BATCHES` | `production_batches` | 缺少 `prod_` 前缀 | 保持现状（已有数据） |
| `FEED_USAGE_RECORDS` | `feed_usage_records` | 缺少 `prod_` 前缀 | 保持现状（已有数据） |

> **说明**：由于这些集合可能已有生产数据，暂不建议重命名。如需重命名，需要执行数据迁移。
