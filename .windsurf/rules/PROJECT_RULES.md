...
Component({
  methods: {
    handleTap() {
      this.triggerEvent('tap', {data});  // 与原生tap事件冲突！
    }
  }
});
...
Component({
  methods: {
    handleTap() {
      this.triggerEvent('recordtap', {data});  // 避免冲突
    }
  }
});
```
...
**原则**：
- ✅ 自定义事件名必须与原生事件区分（tap、touchstart等）
- ✅ 使用业务相关的前缀（如record、item、card等）
- ❌ 禁止使用tap、click、touch等原生事件名

---

#### 8.1.2 组件样式关注点分离

**禁止在组件内定义外部margin** 🔴 严重违规

```scss
// ❌ 错误：组件定义外部间距
.component {
  margin: 20rpx;  // 违反关注点分离原则
  margin-bottom: 20rpx;  // 导致不同页面布局不一致
}
...
.list-container {
  ...
  gap: 20rpx;  // 使用gap统一控制间距
}
```

**原则**：
- ✅ 组件内部：padding、border、background、border-radius
- ✅ 组件外部：由父容器的margin、gap、position控制
- ❌ 组件禁止定义：margin、position（absolute/fixed）
...
```wxml
<!-- 使用组件（连字符传递） -->
<my-component record-id="{{item.id}}" />
```

**原则**：
- ✅ properties定义：使用驼峰命名（recordId）
- ✅ WXML传递：使用连字符（record-id）
- ✅ 微信小程序会自动转换

---

### 8.2 组件化改造流程规范 ⚠️ 血的教训
...
// ❌ 错误：添加调试代码导致性能问题
handleTap() {
  wx.vibrateShort();  // 导致页面卡死！
  wx.showToast({title: '调试信息'});  // 影响用户体验
  console.log('大量调试信息');  // 影响性能
}
...
- ✅ 提交前必须移除所有调试代码
- ✅ 使用条件编译隔离调试代码
- ❌ 禁止在生产代码中保留wx.vibrateShort()
- ❌ 禁止在生产代码中保留调试toast
- ❌ 禁止在生产代码中保留大量console.log
...
# 2. 找到改造前的稳定提交
git log --oneline | grep "改造前"

# 3. 硬回滚到稳定提交
git reset --hard <commit-hash>

# 4. 清理相关文件
rm -rf components/problem-component
rm -rf docs/problem-*
